 ...
 <div className="flex  flex-col md:flex-col gap-2">
  ...
  <div className="border border-white/10 rounded-lg overflow-x-hidden">
    ...
    {/* Header */}
    <div className="grid grid-cols-2 md:grid-cols-4 px-4 py-2 bg-white/5 text-xs text-gray-400 uppercase tracking-wider">
      <span>Prop</span>
      <span>Type</span>
      <span>Required</span>
    ...
    {[
      ...
      <div key={row.prop} className="grid grid-cols-2 md:grid-cols-4 px-4 py-3 border-t border-white/5 text-sm hover:bg-white/5">
        <span className="font-mono text-gray-400">{row.prop}</span>
        <span className="text-gray-400">{row.type}</span>
        <span className=" hidden md:block text-gray-400">{row.required}</span>
        <span className="hidden md:block text-gray-300">{row.desc}</span>
      </div>
    ))}
